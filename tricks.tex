...
\ref{sec:exports}. If a function \var{fn} is called in protected mode, the call
will only succeed if \var{fn} is considered ``safe'', and will otherwise fail 
with the message ``Ignoring call to unsafe function \var{fn} in restricted 
mode.''.
...
\fnref{WMPlex.managed_i} iterates over all regions managed by an \var{mplex} and 
calls \var{iterfn} on each of these regions. If \var{iterfn} was able to alter 
the list of regions managed by \var{mplex}, say by adding additional regions, 
...
\code{WMPlex.managed_i(mplex, iterfn)}, which fails if \var{iterfn} is 
