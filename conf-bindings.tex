...
\begin{itemize}
    ...
    \item \fnref{submap}\code{(keyspec, \{ ... more key bindings ... \})}, and
    \item \fnref{submap_enter}\code{(handler [, guard])}.
\end{itemize}
Mouse actions:
\begin{itemize}
...
(\fnref{kpress}) in the submap. The \fnref{submap_enter} handler
is called when the submap is entered, i.e. when the key for the
submap is pressed.
