...
\centerurl{http://lua-users.org/wiki/FunctionsTutorial}
...
There is one extra file in \code{SHAREDIR} that you may find useful and
...
\file{ion-bindings.lua} which contains the statements to configure 
...
some programs abuse this property) that have a parent window that they
...
Role and name can be retrieved similarly (see the documentation
for \fnref{WClientWin.get_ident}), but may not always be set.
...
instances or roles for the windows, so we'll have to rely an ugly title match.
