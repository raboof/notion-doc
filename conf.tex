...
This chapter should help your configure Notion to your liking. As you
probably already know, Notion uses Lua as a configuration and extension 
...
configuration file \file{cfg\_notion.lua} is provided in section 
\ref{sec:walkthrough}.
...
Notion stores its stock configuration files in
\file{/usr/local/etc/notion/} unless you, the OS package maintainer or 
...
\file{system.mk}\index{system.mk@\file{system.mk}} before compiling Notion.
...
the command \code{locate cfg_notion.lua} might help provided \code{updatedb} 
has been run recently. 

User configuration files go in \file{\~{}/.notion/}. 
Notion always searches the user configuration file directory before the stock
...
files in-place as subsequent installs of Notion will restore the stock
configuration files. Instead you should always make a copy of the stock
file in \file{\~{}/.notion/} and modify this file. For sake of maintainability
...
entirely within \file{cfg\_notion.lua}.
...
\begin{tabularx}{\linewidth}{
    ...
    File & Description \\
    \hline
    \file{cfg\_notion.lua} & 
    The main configuration file \\
    %
    \file{cfg\_notioncore.lua} & 
    Configuration file for Notion's core library.
...
\section{A walk through \file{cfg\_notion.lua}}
\label{sec:walkthrough}

As already mentioned \file{cfg\_notion.lua} is Notion's main configuration
...
take a walk through the stock \file{cfg\_notion.lua}.
...
ALTMETA=""
\end{verbatim}
These settings cause most of Notion's key bindings to use \key{Mod1} as the
...
variables used in the configuration files only, and not Notion settings. 
...
This will, unfortunately, cause Notion to also grab the X server and has some
side effects. 

There are some other options as well; see the documentation
for \fnref{ioncore.set} for details.

As a next step, in the actual \file{cfg\_notion.lua} file, we load
...
under Notion are collected in \file{cfg\_kludges.lua}; see section
